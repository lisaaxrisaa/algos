...
function evenlySpaced(a, b, c) {
  ...
  // Check if the difference between the first and second is the same as the second and third
...
function makeChocolate(small, big, goal) {
  // Use as many big bars as possible without going over the goal
  let maxBigBars = Math.min(big, Math.floor(goal / 5));

  // Subtract big bar weight from the goal to see what's left
  let remaining = goal - maxBigBars * 5;

  // If we have enough small bars to cover what's left, return how many we need
  if (small >= remaining) {
    return remaining;
  } else {
    // Not enough small bars to reach goal
    return -1;
  }
}
